...
impl WasmValue {
    ...
    ///
    /// Notice that the values generated by this function are only meaningful when the [bulk_memory_operations](crate::config::Config::bulk_memory_operations) or [reference_types](crate::config::Config::reference_types) option is enabled in the [Config](crate::Config).
    ///
    /// # Argument
    ///
    ...
    pub fn from_null_ref(ref_ty: RefType) -> Self {
        Self {
            ...
            ty: match ref_ty {
                ...
                RefType::TypedFuncRef => ValType::TypedFuncRef,
                RefType::TypedFuncRefNull => ValType::TypedFuncRefNull,
            },
        }
    }
    ...
    ///
    /// Notice that the [WasmValue]s generated from [FuncRef](crate::FuncRef)s are only meaningful when the [bulk_memory_operations](crate::config::Config::bulk_memory_operations) or [reference_types](crate::config::Config::reference_types) option is enabled in the [Config](crate::Config).
    ///
    /// # Argument
    ///
    ...
    ///
    /// Notice that the [WasmValue]s generated by this function are only meaningful when the `reference_types` option is enabled in
...
impl From<ffi::WasmEdge_Value> for WasmValue {
    fn from(raw_val: ffi::WasmEdge_Value) -> Self {
        if unsafe { ffi::WasmEdge_ValTypeIsI32(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::I32,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsI64(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::I64,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsF32(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::F32,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsF64(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::F64,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsV128(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::V128,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsFuncRef(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::FuncRef,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsExternRef(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::ExternRef,
            }
        } else if unsafe { ffi::WasmEdge_ValTypeIsRef(raw_val.Type) } {
            Self {
                ctx: raw_val,
                ty: ValType::TypedFuncRef,
            }
        } else {
            Self {
                ctx: raw_val,
                ty: ValType::TypedFuncRefNull,
            }
        }
    }
}

impl From<ffi::WasmEdge_ValType> for RefType {
    fn from(ty: ffi::WasmEdge_ValType) -> Self {
        if unsafe { ffi::WasmEdge_ValTypeIsFuncRef(ty) } {
            RefType::FuncRef
        } else if unsafe { ffi::WasmEdge_ValTypeIsExternRef(ty) } {
            RefType::ExternRef
        } else if unsafe { ffi::WasmEdge_ValTypeIsRef(ty) } {
            RefType::TypedFuncRef
        } else if unsafe { ffi::WasmEdge_ValTypeIsRefNull(ty) } {
            RefType::TypedFuncRefNull
        } else {
            panic!("[wasmedge-sys] Cannot convert non-reference WasmEdge_ValType into RefType")
        }
    }
}
impl From<RefType> for ffi::WasmEdge_ValType {
    fn from(ty: RefType) -> Self {
        match ty {
            RefType::FuncRef => unsafe { ffi::WasmEdge_ValTypeGenFuncRef() },
            RefType::ExternRef => unsafe { ffi::WasmEdge_ValTypeGenExternRef() },
            RefType::TypedFuncRef => unimplemented!("TypedFuncRef is not supported yet"),
            RefType::TypedFuncRefNull => unimplemented!("TypedFuncRefNull is not supported yet"),
        }
    }
}
...
impl From<ffi::WasmEdge_ValType> for ValType {
    fn from(ty: ffi::WasmEdge_ValType) -> Self {
        if unsafe { ffi::WasmEdge_ValTypeIsI32(ty) } {
            ValType::I32
        } else if unsafe { ffi::WasmEdge_ValTypeIsI64(ty) } {
            ValType::I64
        } else if unsafe { ffi::WasmEdge_ValTypeIsF32(ty) } {
            ValType::F32
        } else if unsafe { ffi::WasmEdge_ValTypeIsF64(ty) } {
            ValType::F64
        } else if unsafe { ffi::WasmEdge_ValTypeIsV128(ty) } {
            ValType::V128
        } else if unsafe { ffi::WasmEdge_ValTypeIsFuncRef(ty) } {
            ValType::FuncRef
        } else if unsafe { ffi::WasmEdge_ValTypeIsExternRef(ty) } {
            ValType::ExternRef
        } else if unsafe { ffi::WasmEdge_ValTypeIsRef(ty) } {
...
impl From<ValType> for ffi::WasmEdge_ValType {
    fn from(ty: ValType) -> Self {
        match ty {
            ValType::I32 => unsafe { ffi::WasmEdge_ValTypeGenI32() },
            ValType::I64 => unsafe { ffi::WasmEdge_ValTypeGenI64() },
            ValType::F32 => unsafe { ffi::WasmEdge_ValTypeGenF32() },
            ValType::F64 => unsafe { ffi::WasmEdge_ValTypeGenF64() },
            ValType::V128 => unsafe { ffi::WasmEdge_ValTypeGenV128() },
            ValType::FuncRef => unsafe { ffi::WasmEdge_ValTypeGenFuncRef() },
            ValType::ExternRef => unsafe { ffi::WasmEdge_ValTypeGenExternRef() },
            ValType::TypedFuncRef => unimplemented!("TypedFuncRef is not supported yet"),
            ValType::TypedFuncRefNull => unimplemented!("TypedFuncRefNull is not supported yet"),
        }
    }
}
