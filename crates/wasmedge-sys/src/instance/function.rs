...
pub type HostFn = fn(CallingFrame, Vec<WasmValue>) -> Result<Vec<WasmValue>, HostFuncError>;

extern "C" fn wrap_sync_fn(
    key_ptr: *mut c_void,
    ...
    let real_func: HostFn = unsafe { std::mem::transmute(key_ptr) };
...
impl Function {
    ...
    /// let func = Function::create(&func_ty, real_add, 0).expect("fail to create a Function instance");
    /// ```
    pub fn create(ty: &FuncType, real_fn: HostFn, cost: u64) -> WasmEdgeResult<Self> {
        let ctx = unsafe {
...
mod tests {
    ...
    fn test_func_create_host_func_in_host_func() {
        ...
        let func_ty = result.unwrap();
        // create a host function
        let result = Function::create(&func_ty, func, 0);
        assert!(result.is_ok());
        let host_func = result.unwrap();
